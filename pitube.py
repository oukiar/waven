...
try:
    ...
    print("Using sigle file BeasutifulSoup")
...
class yt_video():
    ...
    def __init__(self,name='NA',link='NA',length='NA',thumbnail="NA"):
...
class yt_list():
    ...
    def get_data(self,main_search=False):
        ...
        for j in soup.findAll('a', href = True):
            ...
            #print "----------------------------------"
            #print j.findAll("span", class="yt-thumb-simple")
            ...
            check_thumbnail = j.findAll("span", attrs={'class':'yt-thumb-simple'} )
            ...
            if main_search == True:
                ...
                if check_vid != -1 and check_list == -1 and check_comment == -1 and number == 0:
                    ...
                    names.append(j.text[0:])
                    number = 0
                    
                elif len(check_thumbnail) > 0:
                    #print("-------------------")
                    #print j
                    
                    img = j.findAll("img")[0]["src"]
        ...
        for k in range (0, len(names)-1):
            ...
            try:
                vid = yt_video(names[k],urls[k],lengths[k], thumbnails[k])
            except:
...
class YoutubeList(Thread):
    '''
    Async search on youtube using a thread
    '''
    def __init__(self, **kwargs):
        
        self.callback = kwargs.pop('callback')
        self.searchtext = kwargs.pop('searchtext')
    ...
    def run(self):
        ...
        self.search = yt_list()
        self.search.find_vids(self.searchtext)
    ...
    def real_callback(self, dt):
        ...
        self.callback(self.search.vid_list)
        self.done = True
...
if __name__ == '__main__':
    ...
    for i in range(0,10):
        search = yt_list()
        search.find_vids("Calle 13 pueblo")
        print(i)
        print(search.vid_list)
